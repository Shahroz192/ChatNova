...
def test_get_user_sessions(client: TestClient, test_user: User):
    ...
    session_data = {
        "title": "My Session",
        "description": "A session to test retrieval"
    }
...
def test_create_chat_message(client: TestClient, test_user: User):
    ...
    assert response.status_code in [200, 500]  # 200 if successful, 500 if AI service fails
...
def test_create_chat_message_with_session(client: TestClient, test_user: User):
    ...
    session_data = {
        "title": "Chat Session",
        "description": "Session for chat testing"
    }
...
def test_access_chat_endpoints_without_auth(client: TestClient):
    ...
    for endpoint in endpoints_to_test:
        if endpoint == "/api/v1/sessions":
            ...
            response = client.post(endpoint, json={"title": "Test", "description": "Test"})
            assert response.status_code == 401, f"Endpoint {endpoint} should require authentication"
        elif endpoint == "/api/v1/chat":
            # Test POST
            response = client.post(endpoint, json={"content": "test", "model": "gemini-2.5-flash"})
            assert response.status_code == 401, f"Endpoint {endpoint} should require authentication"
