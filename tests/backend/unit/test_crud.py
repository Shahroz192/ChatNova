...
def test_user_authentication(db_session: Session):
    ...
    # Create a user
    user_data = {"email": "auth_test@example.com", "password": "TestPassword123"}
    user_in = UserCreate(**user_data)
    created_user = user.create(db_session, obj_in=user_in)
    ...
    wrong_auth = user.authenticate(
        db_session, email=user_data["email"], password="wrongpassword"
    )
...
def test_create_message(db_session: Session):
    ...
    # Create a message
    message_data = {"content": "Hello, world!", "model": "gpt-3.5-turbo"}
...
def test_get_messages_by_user(db_session: Session):
    ...
    message_data_1 = {"content": "First message", "model": "gpt-3.5-turbo"}
    message_data_2 = {"content": "Second message", "model": "gpt-3.5-turbo"}
...
def test_count_messages_by_user(db_session: Session):
    ...
    # Create multiple messages for the user
    message_data_1 = {"content": "Count test message 1", "model": "gpt-3.5-turbo"}
    message_data_2 = {"content": "Count test message 2", "model": "gpt-3.5-turbo"}
...
def test_get_latest_messages(db_session: Session):
    ...
    # Create multiple messages for the user
    for i in range(5):
        message_data = {"content": f"Latest test message {i}", "model": "gpt-3.5-turbo"}
        message.create(
            db_session,
            obj_in=MessageCreate(**message_data),
    ...
    latest_messages = message.get_latest_messages(
        db_session, user_id=created_user.id, limit=3
    )
...
def test_get_user_api_key_by_user_and_model(db_session: Session):
    ...
    retrieved_api_key = user_api_key.get_by_user_and_model(
        db_session, user_id=created_user.id, model_name=api_key_data["model_name"]
    )
...
def test_create_user_mcp_server(db_session: Session):
    ...
    assert (
        created_mcp_server.mcp_servers_config == mcp_server_data["mcp_servers_config"]
    )
