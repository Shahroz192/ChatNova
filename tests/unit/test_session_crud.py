...
def test_create_session(db_session: Session, test_user: User):
    ...
    session_in = ChatSessionCreate(
        title="Test Session", description="A test chat session"
    )
...
def test_get_session(db_session: Session, test_user: User):
    ...
    created_session = crud.session.create(
        db_session, obj_in=session_in, user_id=test_user.id
    )
...
def test_update_session(db_session: Session, test_user: User):
    ...
    updated_session = crud.session.update(
        db_session, db_obj=created_session, obj_in=session_update
    )
...
def test_delete_session(db_session: Session, test_user: User):
    ...
    # Create a session first
    session_in = ChatSessionCreate(
        title="To Delete", description="Session to be deleted"
    )
    created_session = crud.session.create(
        db_session, obj_in=session_in, user_id=test_user.id
    )
...
def test_get_sessions_by_user_with_search(db_session: Session, test_user: User):
    ...
    # Create multiple sessions for the user
    session_in1 = ChatSessionCreate(
        title="Python Help", description="Python programming help"
    )
    session_in2 = ChatSessionCreate(
        title="JavaScript Guide", description="JavaScript guide"
    )
    session_in3 = ChatSessionCreate(
        title="AI Discussion", description="AI-related discussion"
    )
    ...
    sessions = crud.session.get_by_user_with_message_count(
        db_session, user_id=test_user.id, search="Python"
    )
    ...
    sessions = crud.session.get_by_user_with_message_count(
        db_session, user_id=test_user.id, search="Guide"
    )
