...
def test_create_message(db_session: Session, test_user: User):
    ...
    message_in = MessageCreate(content="Hello, this is a test message", model="gpt-4")
...
def test_create_message_with_session(db_session: Session, test_user: User):
    ...
    session = crud.session.create(db_session, obj_in=session_in, user_id=test_user.id)

    # Create a message with the session
    message_in = MessageCreate(
        content="Hello, this is a test message for a session", model="gpt-4"
    )
    message = crud.message.create(
        db_session,
        obj_in=message_in,
...
def test_get_message(db_session: Session, test_user: User):
    ...
    message_in = MessageCreate(content="Test message content", model="gpt-4")
    created_message = crud.message.create(
        db_session, obj_in=message_in, response="Test response", user_id=test_user.id
    )
...
def test_update_message(db_session: Session, test_user: User):
    ...
    # Create a message first
    message_in = MessageCreate(content="Original content", model="gpt-4")
    created_message = crud.message.create(
        db_session,
        obj_in=message_in,
    ...
    updated_message = crud.message.update(
        db_session, db_obj=created_message, obj_in={"response": "Updated response"}
    )
...
def test_delete_message(db_session: Session, test_user: User):
    ...
    # Create a message first
    message_in = MessageCreate(content="Message to delete", model="gpt-4")
    created_message = crud.message.create(
        db_session,
        obj_in=message_in,
...
def test_get_messages_by_user(db_session: Session, test_user: User):
    ...
    crud.message.create(
        db_session, obj_in=message_in1, response="Response 1", user_id=test_user.id
    )
    crud.message.create(
        db_session, obj_in=message_in2, response="Response 2", user_id=test_user.id
    )
...
def test_count_messages_by_user(db_session: Session, test_user: User):
    ...
    crud.message.create(
        db_session, obj_in=message_in3, response="Response 3", user_id=test_user.id
    )
...
def test_get_messages_by_user_with_session_filter(db_session: Session, test_user: User):
    ...
    messages = crud.message.get_by_user(
        db_session, user_id=test_user.id, session_id=session.id
    )
...
def test_get_messages_by_user_with_search(db_session: Session, test_user: User):
    ...
    crud.message.create(
        db_session, obj_in=message_in1, response="Python response", user_id=test_user.id
    )
    crud.message.create(
        db_session,
        obj_in=message_in2,
    ...
    crud.message.create(
        db_session, obj_in=message_in3, response="AI response", user_id=test_user.id
    )
